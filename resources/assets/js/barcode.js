var barcode = function() {
	...
	function init() {
		...
		window.URL = window.URL || window.webkitURL;  // This is mostly unnecessary now.
		...
		if (typeof config.start !== 'number' || config.start < 0 || config.start > 1) {
			config.start = 0;
		}
		if (typeof config.end !== 'number' || config.end < 0 || config.end > 1) {
			config.end = 1;
		}
